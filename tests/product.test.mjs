...
test("mantém as 44 aulas de JavaScript, Python e C++", async () => {
  ...
  assert.match(data, /javascriptStudyGuides/);
  assert.match(data, /keyPoints:/);
  assert.match(data, /commonMistake:/);
  assert.match(data, /guidedPractice:/);
  assert.match(app, /LEITURA GUIADA · CONTEÚDO NEXACODE/);
  assert.match(app, /currentLesson\.studyGuide/);
  assert.doesNotMatch(app, /Assistir à playlist/);
  assert.doesNotMatch(data, /youtube\.com/);
  const studyGuideBlock = data.slice(
    data.indexOf("const javascriptStudyGuides"),
    data.indexOf("const lesson ="),
  );
  assert.equal(
    (studyGuideBlock.match(/\n  (?:"[^"]+"|[a-z]+): \{/g) ?? []).length,
    24,
  );
